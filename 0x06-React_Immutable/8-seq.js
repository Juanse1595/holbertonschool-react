...
export default function printBestStudents(obj) {
  const bestStudents = Immutable.Seq(obj).filter((elm) => elm.score > 70);
  const bestStudentsObj = bestStudents.toJS();
  Object.keys(bestStudentsObj).forEach((key) => {
    bestStudentsObj[key].firstName = bestStudentsObj[key].firstName.charAt(0).toUpperCase()
    + bestStudentsObj[key].firstName.slice(1);
    bestStudentsObj[key].lastName = bestStudentsObj[key].lastName.charAt(0).toUpperCase()
